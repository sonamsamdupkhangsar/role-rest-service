...
create table if not exists Authz_Manager_Role_Assignment(
    id uuid primary key,
    authz_manager_role_id uuid,
    user_id uuid,
    scope_type varchar,
    scope_id uuid
);
create unique index if not exists Authz_Manager_Role_Assignment_Uidx
    on Authz_Manager_Role_Assignment(authz_manager_role_id, user_id, scope_type, scope_id);
